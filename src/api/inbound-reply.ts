...
router.post("/webhooks/inbound-email", async (req: Request, res: Response) => {
  ...
  try {
    ...
    // =====================================================================
    // Update lead with reply data (same for all sentiments)
    // =====================================================================
    const newStatus =
    ...
    // =====================================================================
    // Route ALL sentiments through Teams — nothing auto-sends
    // =====================================================================
    ...
    if (sentiment.sentiment === "positive") {
      ...
      try {
        await chatSvc.sendWarmReplyAlert(updated, sentiment.reasoning, sentiment.suggested_action);
      } catch (e) {
      ...
      try {
        ...
        await chatSvc.sendReplyApprovalCard({
          ...
          draftPreview: stripHtml(goodbye.email_body_html).slice(0, 200),
          approveUrl: approveUrl.replace("tier1_approve", "followup_approve"),
          rejectUrl: rejectUrl.replace("tier1_reject", "followup_reject"),
        });
      } catch (e) {
        ...
        try {
          await chatSvc.sendReplyAlert(updated, sentiment.sentiment, sentiment.reasoning, textBody.slice(0, 300));
        } catch { /* swallow */ }
      }
      ...
      try {
        await chatSvc.sendReplyAlert(updated, "soft_no", sentiment.reasoning, textBody.slice(0, 300));
      } catch (e) {
      ...
      try {
        await chatSvc.sendReplyAlert(updated, "neutral", sentiment.reasoning, textBody.slice(0, 300));
      } catch (e) {
